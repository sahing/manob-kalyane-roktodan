...
@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ newRequestModal: false, viewMode: 'grid', inquiryModal: false, selectedPatientName: '', selectedBloodGroup: '' }">
            ...
            <div class="glass-card p-6 rounded-2xl border-rose-900/20 dark:border-rose-900/30 hover:border-rose-600/60 transition shadow-lg flex flex-col justify-between">
                ...
                <div class="pt-4 border-t border-slate-200 dark:border-slate-800 space-y-3">
                    <div class="flex items-center justify-between gap-1.5 flex-wrap">
                        ...
                        @else
                            <button type="button" @click="selectedPatientName = '{{ addslashes($req->patient_name) }}'; selectedBloodGroup = '{{ $req->blood_group }}'; inquiryModal = true" class="px-2.5 py-2 rounded-xl text-xs font-extrabold bg-emerald-600 hover:bg-emerald-500 text-white transition flex items-center gap-1 shadow-md">
                                📞 Call
                            </button>
                            <button type="button" @click="selectedPatientName = '{{ addslashes($req->patient_name) }}'; selectedBloodGroup = '{{ $req->blood_group }}'; inquiryModal = true" class="px-2.5 py-2 rounded-xl text-xs font-extrabold bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 transition flex items-center gap-1">
                                💬 WA
                            </button>
                        @endif
                        ...
                        <tr class="hover:bg-slate-100/60 dark:hover:bg-slate-900/60 transition">
                            ...
                            <td class="p-4 text-right">
                                <div class="inline-flex items-center space-x-1.5">
                                    ...
                                    @else
                                        <button type="button" @click="selectedPatientName = '{{ addslashes($req->patient_name) }}'; selectedBloodGroup = '{{ $req->blood_group }}'; inquiryModal = true" class="px-2.5 py-1.5 rounded-lg text-xs font-bold bg-emerald-600 hover:bg-emerald-500 text-white transition flex items-center gap-1">
                                            📞 Call
                                        </button>
                                        <button type="button" @click="selectedPatientName = '{{ addslashes($req->patient_name) }}'; selectedBloodGroup = '{{ $req->blood_group }}'; inquiryModal = true" class="px-2.5 py-1.5 rounded-lg text-xs font-bold bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 transition flex items-center gap-1">
                                            💬 WA
                                        </button>
                                    @endif
