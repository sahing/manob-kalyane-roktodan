...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="adminDashboard()">
    ...
    @php
        ...
        if (!request()->has('tab') && !request()->hasAny(['search_user', 'users_page', 'per_page']) && !$authUser->hasPermission('manage_requests')) {
            if ($authUser->hasPermission('manage_finances')) {
                ...
                $defaultTab = 'users';
            } elseif ($authUser->hasPermission('manage_content')) {
                $defaultTab = 'blog';
            } elseif ($authUser->hasPermission('view_analytics')) {
                $defaultTab = 'analytics';
            } elseif ($authUser->hasPermission('record_donations')) {
                $defaultTab = 'donations';
            }
        }
    @endphp
    ...
    <div class="flex items-center space-x-2 overflow-x-auto pb-4 mb-8 text-xs font-bold border-b border-slate-200 dark:border-slate-800">
        ...
        @if($authUser->hasPermission('manage_content'))
        ...
        @if($authUser->hasPermission('view_analytics'))
        ...
        @if($authUser->hasPermission('record_donations'))
        ...
        @if($authUser->hasPermission('manage_content') || $authUser->isAdmin())
        ...
        @if($authUser->hasPermission('manage_users') || $authUser->isAdmin())
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <div class="w-8 h-8 rounded-lg bg-rose-600 text-white font-extrabold flex items-center justify-center text-xs">
                                            ...
                                            <div class="flex items-center gap-1.5">
                                                ...
                                                <span class="px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase {{ $u->role === 'admin' ? 'bg-amber-500/20 text-amber-500 border border-amber-500/30' : ($u->canAccessAdmin() ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300') }}">
                ...
                <table class="w-full text-left text-xs">
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800 font-medium">
                        @forelse($inquiries as $inq)
                            <tr class="hover:bg-slate-100/60 dark:hover:bg-slate-900/60 cursor-pointer transition" @click="openInquiryDetailModal({{ json_encode([
                                'id' => $inq->id,
                                'name' => $inq->name,
                                'phone' => $inq->phone,
                                'purpose' => $inq->purpose,
                                'ip_address' => $inq->ip_address,
                                'logged_at' => $inq->created_at->format('d M Y, h:i:s A') . ' (' . $inq->created_at->diffForHumans() . ')'
                                ...
                                <td class="p-3.5 font-bold text-slate-900 dark:text-white">
                                    <div class="flex items-center gap-2">
                                        <span>{{ $inq->name }}</span>
                                        <span class="px-2 py-0.5 rounded text-[9px] font-extrabold bg-rose-500/10 text-rose-600 border border-rose-500/20">NEW INQUIRY</span>
                                    </div>
                                </td>
                                ...
                                <td class="p-3.5 text-slate-700 dark:text-slate-300 max-w-xs truncate">
                                    <span class="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/20 text-amber-600 dark:text-amber-300">
                                ...
                                <td class="p-3.5 text-right" @click.stop>
                                    <div class="flex items-center justify-end gap-1.5">
                                        <!-- View Detailed Modal -->
                                        <button type="button" @click="openInquiryDetailModal({{ json_encode([
                                            'id' => $inq->id,
                                            'name' => $inq->name,
                                            'phone' => $inq->phone,
                                            'purpose' => $inq->purpose,
                                            'ip_address' => $inq->ip_address,
                                            'logged_at' => $inq->created_at->format('d M Y, h:i:s A') . ' (' . $inq->created_at->diffForHumans() . ')'
                                        ], JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP) }})" class="px-2.5 py-1.5 rounded-xl text-[11px] font-extrabold bg-slate-800 text-white hover:bg-slate-700 transition flex items-center gap-1 shadow">
                                            👁️ View Details
                                        </button>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 p-4 rounded-2xl bg-slate-100 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800">
                        @foreach($availablePermissions as $key => $title)
                            <label class="flex items-start space-x-3 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/60 cursor-pointer hover:border-rose-500/50 transition">
                                <input type="checkbox" name="permissions[]" value="{{ $key }}" x-model="editingRole.permissions" class="mt-0.5 rounded border-slate-300 text-rose-600 focus:ring-rose-500 w-4 h-4">
                                <div>
        ...
        <div class="glass-card max-w-xl w-full p-6 sm:p-8 rounded-3xl border border-slate-300 dark:border-slate-800 relative shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <form :action="'/admin/users/' + editUser.id" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1 uppercase">Last Donation Date</label>
                        <input type="date" name="last_donation_date" x-model="editUser.last_donation_date" class="w-full bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white">
                    </div>
                </div>
    ...
    <!-- Admin Inquiry Request Detailed View Modal -->
    <div x-show="inquiryDetailModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div class="glass-card max-w-xl w-full p-6 sm:p-8 rounded-3xl border border-slate-300 dark:border-slate-800 relative shadow-2xl space-y-6">
            <button @click="inquiryDetailModal = false" class="absolute top-5 right-5 w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-500 hover:text-slate-900 dark:hover:text-white flex items-center justify-center font-bold text-lg transition">&times;</button>

            <div class="flex items-center space-x-3">
                <span class="w-12 h-12 rounded-2xl bg-rose-600/20 text-rose-500 border border-rose-500/30 flex items-center justify-center text-2xl shadow-inner">
                    🩸
                </span>
                <div>
                    <h3 class="text-xl font-extrabold text-slate-900 dark:text-white" x-text="selectedInquiry ? selectedInquiry.name : 'Inquiry Details'"></h3>
                    <span class="text-xs text-rose-600 dark:text-rose-400 font-bold uppercase tracking-wider">Real-Time Contact & Blood Request Inquiry</span>
                </div>
            </div>

            <template x-if="selectedInquiry">
                <div class="space-y-4 text-xs text-slate-700 dark:text-slate-300">
                    <div class="grid grid-cols-2 gap-3 p-4 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                        <div>
                            <span class="text-[10px] uppercase font-bold text-slate-400 block">Requester Name</span>
                            <span class="font-extrabold text-sm text-slate-900 dark:text-white" x-text="selectedInquiry.name"></span>
                        </div>
                        <div>
                            <span class="text-[10px] uppercase font-bold text-slate-400 block">Contact Phone Number</span>
                            <span class="font-mono font-extrabold text-sm text-rose-600 dark:text-rose-400" x-text="selectedInquiry.phone"></span>
                        </div>
                    </div>

                    <div class="p-4 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-1">
                        <span class="text-[10px] uppercase font-bold text-slate-400 block">Purpose / Emergency Request Details</span>
                        <div class="text-sm font-bold text-slate-900 dark:text-white leading-relaxed" x-text="selectedInquiry.purpose || 'Donor Contact Access'"></div>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60">
                            <span class="text-[10px] uppercase font-bold text-slate-400 block">Time Logged</span>
                            <span class="font-semibold text-slate-800 dark:text-slate-200" x-text="selectedInquiry.logged_at || selectedInquiry.created_at"></span>
                        </div>
                        <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60">
                            <span class="text-[10px] uppercase font-bold text-slate-400 block">IP Address</span>
                            <span class="font-mono text-slate-800 dark:text-slate-200" x-text="selectedInquiry.ip_address || 'Unknown'"></span>
                        </div>
                    </div>

                    <div class="pt-2 border-t border-slate-200 dark:border-slate-800">
                        <span class="text-[11px] font-extrabold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3 block">Direct Communication Options for Admin:</span>
                        <div class="grid grid-cols-3 gap-2">
                            <a :href="'tel:' + selectedInquiry.phone" class="px-3 py-2.5 rounded-xl text-xs font-extrabold bg-emerald-600 hover:bg-emerald-500 text-white transition flex items-center justify-center gap-1.5 shadow">
                                📞 Call Phone
                            </a>
                            <a :href="'https://wa.me/91' + (selectedInquiry.phone ? selectedInquiry.phone.replace(/[^0-9]/g, '') : '') + '?text=Hello%20' + encodeURIComponent(selectedInquiry.name || '') + ',%20responding%20from%20Manab%20Kalyane%20Rokto%20Dan%20regarding%20your%20blood%20request%20inquiry...'" target="_blank" class="px-3 py-2.5 rounded-xl text-xs font-extrabold bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/30 transition flex items-center justify-center gap-1.5">
                                💬 WhatsApp
                            </a>
                            <button type="button" @click="inquiryDetailModal = false; openPortalChat({ name: selectedInquiry.name, phone: selectedInquiry.phone, notes: selectedInquiry.purpose })" class="px-3 py-2.5 rounded-xl text-xs font-extrabold bg-indigo-600 hover:bg-indigo-500 text-white transition flex items-center justify-center gap-1.5 shadow">
                                💬 Portal Chat
                            </button>
                        </div>
                    </div>
...
function adminDashboard() {
    return {
        ...
        editUserModal: false,
        editUser: {},
        editingRole: { id: null, name: '', label: '', description: '', permissions: [] },
        selectedInquiry: null,
        inquiryDetailModal: false,

        openEditRoleModal(roleData) {
            this.editingRole = {
                id: roleData.id,
                name: roleData.name,
