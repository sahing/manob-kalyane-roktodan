...
@section('content')
<div class="max-w-md mx-auto px-4 py-12" x-data="{ mode: '{{ ($errors->has('name') || $errors->has('phone') || $errors->has('password_confirmation') || old('block')) ? 'signup' : 'login' }}' }">
        ...
        <div x-show="mode === 'login'">
            <form action="{{ route('login') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" required autofocus class="w-full bg-slate-100 dark:bg-slate-900 border @error('email') border-rose-500 focus:ring-rose-500 @else border-slate-300 dark:border-slate-700 focus:border-rose-500 @enderror rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none">
                    @error('email')
            ...
            <form action="{{ route('register') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" required class="w-full bg-slate-100 dark:bg-slate-900 border @error('email') border-rose-500 @else border-slate-300 dark:border-slate-700 @enderror rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-rose-500">
                        @error('email')
                            <span class="text-rose-500 text-[11px] font-bold mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="tel" name="phone" value="{{ old('phone') }}" required class="w-full bg-slate-100 dark:bg-slate-900 border @error('phone') border-rose-500 @else border-slate-300 dark:border-slate-700 @enderror rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-rose-500">
                        @error('phone')
                            <span class="text-rose-500 text-[11px] font-bold mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
