...
@section('title', 'Register / Sign In — Manab Kalyane Rokto Dan')

@section('content')
<div class="max-w-md mx-auto px-4 py-12" x-data="{ mode: 'signup' }">
    <div class="glass-card p-8 rounded-3xl border border-slate-300 dark:border-slate-800 shadow-2xl">
        ...
        @if(!empty($refCode))
            <div class="mb-4 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 text-xs font-bold flex items-center gap-2">
                <span>🎁 Inviter Referral Code Applied: <code class="font-mono text-white bg-emerald-700/40 px-2 py-0.5 rounded">{{ $refCode }}</code> (+20 Bonus Points)</span>
            </div>
        @endif
            ...
            <form action="{{ route('register') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" required class="w-full bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-rose-500">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Phone</label>
                        <input type="tel" name="phone" value="{{ old('phone') }}" required class="w-full bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-rose-500">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                ...
                <div>
                    ...
                    <input type="text" name="ref_code" value="{{ old('ref_code', $refCode ?? '') }}" placeholder="e.g. MKRD-REF-0001" class="w-full bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-xs font-mono font-bold text-slate-900 dark:text-white uppercase focus:outline-none focus:border-rose-500">
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
