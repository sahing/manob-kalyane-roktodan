...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ shareModal: false }">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 pb-8 border-b border-slate-200 dark:border-slate-800">
        <div>
            <span class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-rose-500/10 dark:bg-rose-500/20 text-rose-600 dark:text-rose-300 border border-rose-500/30 mb-3">
                <span class="w-2 h-2 rounded-full bg-rose-500 animate-ping"></span>
                Community Voices & Inspiration
            </span>
            <h1 class="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Donor Stories & Experiences</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400 mt-2 max-w-2xl">Read real stories from voluntary blood donors across Bhagwangola & Murshidabad who give the gift of life.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($stories as $story)
            <div class="glass-card rounded-3xl overflow-hidden hover:border-rose-600/60 transition shadow-xl flex flex-col justify-between group" x-data="{
                ...
                <div>
                    ...
                    @if($story->photo_url)
                        <a href="{{ route('stories.show', $story->id) }}" class="block relative h-56 overflow-hidden bg-slate-900">
                    ...
                    <div class="p-6">
                        <div class="flex items-center space-x-3 mb-3">
                            @if(!$story->photo_url)
                                <div class="w-11 h-11 rounded-xl bg-rose-600/20 border border-rose-500/30 text-rose-600 dark:text-rose-400 font-extrabold text-base flex items-center justify-center shadow-md">
                                    {{ $story->blood_group ?? '🩸' }}
                                </div>
                            @endif
                            <div>
                                <h3 class="text-sm font-bold text-slate-900 dark:text-white">{{ $story->donor_name }}</h3>
                        ...
                        <h2 class="text-base font-extrabold text-slate-900 dark:text-white mb-2 group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">
                            <a href="{{ route('stories.show', $story->id) }}">"{{ $story->title }}"</a>
                        </h2>
                        
                        <p class="text-xs text-slate-600 dark:text-slate-300 leading-relaxed bg-slate-100 dark:bg-slate-900/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800/60 whitespace-pre-line line-clamp-4">
                            {{ $story->experience }}
                        </p>
                    </div>
                </div>

                <!-- Interaction Toolbar (Likes, Comments, Share, Detail Page) -->
                <div class="px-6 py-4 border-t border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/40 flex items-center justify-between text-xs text-slate-500">
                    <div class="flex items-center space-x-3">
                        <!-- Like Button -->
                        <button @click="toggleLike()" class="flex items-center gap-1.5 font-bold transition"
                            :class="liked ? 'text-rose-600 dark:text-rose-400' : 'text-slate-500 hover:text-rose-500'">
                            <span x-text="liked ? '❤️' : '🤍'"></span>
                            <span x-text="likesCount"></span>
                        </button>

                        <!-- Comments Counter -->
                        <a href="{{ route('stories.show', $story->id) }}#comments" class="flex items-center gap-1.5 font-bold hover:text-slate-900 dark:hover:text-white transition">
                            💬 {{ $story->comments_count }}
                        </a>

                        <!-- Share Counter -->
                        <a href="{{ route('stories.show', $story->id) }}" class="flex items-center gap-1.5 font-bold hover:text-slate-900 dark:hover:text-white transition">
                            🔗 {{ $story->shares_count }}
                        </a>
                    </div>

                    <a href="{{ route('stories.show', $story->id) }}" class="font-extrabold text-rose-600 dark:text-rose-400 hover:underline">
                        Read Story →
                    </a>
                </div>
            </div>
        @empty
            <div class="col-span-3 text-center p-12 glass-card rounded-2xl text-slate-500 dark:text-slate-400">
                <div class="w-14 h-14 rounded-2xl bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 flex items-center justify-center mx-auto mb-3 text-2xl font-bold">
                    ✍️
                </div>
                <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-1">No Donor Stories Shared Yet</h3>
                <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">Be the first voluntary donor to share your donation story and inspire others!</p>
                <button @click="shareModal = true" class="px-5 py-2.5 rounded-xl text-xs font-bold bg-rose-600 hover:bg-rose-500 text-white transition shadow">
    ...
    <div x-show="shareModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div class="glass-card max-w-lg w-full p-6 sm:p-8 rounded-2xl border border-slate-300 dark:border-slate-800 relative shadow-2xl overflow-y-auto max-h-[90vh]">
            <button @click="shareModal = false" class="absolute top-4 right-4 text-slate-400 hover:text-slate-900 dark:hover:text-white text-2xl font-bold">&times;</button>
            ...
            <form action="{{ route('stories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="donor_name" value="{{ Auth::user()?->name }}" required placeholder="e.g. Tariqul Islam" class="w-full bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-rose-500">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input type="text" name="location" placeholder="e.g. Bhagwangola / Murshidabad Medical College" class="w-full bg-slate-100 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-rose-500">
                </div>
